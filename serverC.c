...
#define MYPORT "23341"    // the port ServerC will be connecting to
...
int main(void)
{
    ...
    printf("%s\n","The Server C is up and running using UDP on port 23341" );
    ...
    recvfrom(sockfd, arrayReceived, sizeof(arrayReceived), 0, (struct sockaddr *)&their_addr, &addr_len); //Beej Guide reference
    int numbersRecvd = ntohl(arrayReceived[0]);
    printf("The Server C has received %d numbers\n", numbersRecvd);
    int i = 0;
    for (i = 0 ; i < numbersRecvd ; ++i) { //Make it dynamic and print how many numbers you received.
    ...
    //strcat(sendResult ,"-ServerC");
    printf("The Server C has successfully finished the reduction %s : %s\n", buf,sendResult);
